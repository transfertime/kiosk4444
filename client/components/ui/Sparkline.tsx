...
export default function Sparkline({ data, color = "#06b6d4", width = 100, height = 24, strokeWidth = 1.5 }: { data: number[]; color?: string; width?: number; height?: number; strokeWidth?: number }) {
  ...
  return (
    <div ref={wrapperRef} className="relative" style={{ width, height }} onMouseMove={onMove} onMouseLeave={onLeave}>
      ...
      {tooltip && (
        <div style={{ position: "absolute", left: Math.min(Math.max(0, tooltip.x - 30), width - 60), top: tooltip.y - 36 }} className="bg-white dark:bg-neutral-900 border border-white/10 text-xs rounded px-2 py-1 shadow">
          {tooltip.value}
        </div>
      )}
    </div>
